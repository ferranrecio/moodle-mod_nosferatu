...
class Mutations {
    ...
    putPerson(stateManager, person) {
        if (person === undefined) {
            person = {};
        }

        const firstFreeId = Array.from(stateManager.state.people.keys()).reduce((max, id) => Math.max(max, id), 0) + 1;
    ...
    removePerson(stateManager, personId) {
        const state = stateManager.state;
        stateManager.setReadOnly(false);
